...
describe('sync-docs-index', () => {
  it('--help exits 0', async () => {
    const r = await runShim('sync-docs-index', ['--help']);
    expect(r.exitCode).toBe(0);
  });

  it('--version exits 0', async () => {
    const r = await runShim('sync-docs-index', ['--version']);
    expect(r.exitCode).toBe(0);
  });

  it('unknown flag exits 2', async () => {
    const r = await runShim('sync-docs-index', ['--xxx']);
    expect(r.exitCode).toBe(2);
  });

  it('--dry-run --json emits envelope', async () => {
    const r = await runShim('sync-docs-index', ['--dry-run', '--json']);
    expect(r.exitCode).toBe(0);
